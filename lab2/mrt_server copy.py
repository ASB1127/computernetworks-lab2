...
class Server(MRTBase):
    ...
    def rcv_handler(self):
        while True:
            ...
            if self.state == "SYN_RCVD" and pkt.type == packet.Packet.ACK:
                #if the ack packet is received then the connection is established
                self.state = "ESTABLISHED"
                self.accepted_event.set()
                continue
            
            if self.state == "ESTABLISHED" and pkt.type == packet.Packet.FIN:
                ...
                continue
            if self.state == "FIN_RCVD" and pkt.type == packet.Packet.ACK:
                self.state = "CLOSED"
                self.closed_event.set()
                continue
            
            #Check to see if the packet is a data packet
            if self.state == "ESTABLISHED" and pkt.type == packet.Packet.DATA:
